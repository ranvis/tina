...
- (IBAction)savePlayList:(id)sender
{
	...
	if( (out=fopen([[sPanel filename] UTF8String],"w"))==NULL ){
		NSBeep();
		return;
	}
	for( i=0; i<number_of_files; i++ ){
		fprintf( out, "%s\n", list_of_files[i]->file);
	}
	fclose(out);
}

static void loadPlayList_main(NSString *list_filename)
{
	FILE *in;
	const int buflen=1024;
	char  buf[buflen];
	
	if( (in=fopen([list_filename UTF8String], "r"))==NULL ){
		//NSLog(@"loadPlayList_main: fopen fail.");
		NSBeep();
		return;
	}
	
	//NSLog(@"loadPlayList_main: fopen success.");
...
- (IBAction)loadPlayList:(id)sender
{
    ...
    if (result == NSOKButton) {
		...
		loadPlayList_main([oPanel filename]);

        if( current_no==-1 && prev_number_of_files<number_of_files
			  && mac_peek_rc(&value)==RC_NONE ){
            current_no = prev_number_of_files;
            mac_send_rc(RC_LOAD_FILE, 0);
        }
    }
...
- (void)core_thread:(id)arg
{
    ...
    char *argv[] = {"timidity", "-c", "./timidity.cfg"};
