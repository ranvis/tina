...
static void dev_remake_disp(Rect rect)
{    
    if( wrdEnv.gon_flag){
...
{			//ok 0
    ...
    if( ! magimg ){
        ctl->cmsg(CMSG_INFO, VERB_VERBOSE, "macosx_mag_load NG:%s",filename);
        return -1;
    }
...
/***********************************************************************/
static void dev_init(int version)
{
    int i;
