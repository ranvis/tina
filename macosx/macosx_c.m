...
static void ctl_refresh(void)
{
    static double  last_refresh_secs=0;
    static bool    request_pend=0;
    
    if( last_refresh_secs +1.0 < get_current_calender_time() ){
	[macosx_controller refresh];
	last_refresh_secs = get_current_calender_time();
	request_pend = FALSE;
    }else{
	request_pend = TRUE;
    }
...
void ctl_load_file(const char* fn)
{
    ...
    //}
    if( nfiles && (!mac_play_active) && mac_peek_rc(&value)==RC_NONE ){
        current_no = prev_number_of_files;
        mac_send_rc(RC_CONTINUE, 0);
		usleep(500000); //500ms
    }
...
static void ctl_event(CtlEvent *e)
{
    DEBUG_MACOSX_C((CMSG_INFO, VERB_VERBOSE, "ctl_event %d", e->type));
...
static void ctl_total_time(int tt)
{
    ...
    list_of_files[current_no]->total_time = secs;
    [macosx_controller total_time:secs];
#if 0
...
static int ctl_read(int32 *valp)
{
    //DEBUG_MACOSX_C((CMSG_INFO, VERB_VERBOSE, "ctl_read"));
    return mac_get_rc(valp, 0);    	    
}
...
static BOOL file_exist(const char *filename)
{
    ...
    //ctl.cmsg(CMSG_INFO, VERB_NORMAL, "checking %s", filename);
...
static void check_MFnode_WRDfile(MFnode *mfp)
{
    char   wrdfile[512];
    int    len;

    
    strcpy(wrdfile,mfp->file);
    len = strlen(wrdfile);
    mfp->haveWRD = MFNODE_HAVEWRD_NON; //initialize
    if( len <3 ){  //too short to check
	return;
    }
    
    //check neoWRD
    strcpy( &wrdfile[len-3], "nbf");
    if( file_exist( wrdfile) ){
	mfp->haveWRD = MFNODE_HAVEWRD_NEOWRD;
	return;
    }

    //check SWRD
    strcpy( &wrdfile[len-3], "sry");
    if( file_exist( wrdfile) ){
	mfp->haveWRD = MFNODE_HAVEWRD_SWRD;
	return;
    }

    //check WRD
    strcpy( &wrdfile[len-3], "wrd");
    if( file_exist( wrdfile) ){
	mfp->haveWRD = MFNODE_HAVEWRD_WRD;
	return;
    }
}
...
const char *fullpath2filename(const char *fullpath)
{
    const char *filename;
	...
	filename = strrchr(fullpath, '#');
        if( filename==NULL ){
            filename = strrchr(fullpath, PATH_SEP);
	}
	if( filename==NULL ){
	    filename = "--";
	}else{
	    	...
	    	//mac_pre_play();
                {
					...
					:fullpath2filename(list_of_files[current_no]->file))];
		    if( list_of_files[current_no]->haveWRD ){
			[macosx_controller  selectWRD];
		    }
                    mac_play_active=1;
