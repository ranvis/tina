...
static void setDefault(Pref_data* data)
{
    ...
    data->ModulationEnvelope = 0;

#ifdef TEMPER_CONTROL_ALLOW
    data->TemperControl = 1;
#else
    data->TemperControl = 0;
...
static void getPreference_fromGlobals(Pref_data* data)
{
    ...
    data->ModulationEnvelope = opt_modulation_envelope;
    data->TemperControl = opt_temper_control;
	data->Reverb        = opt_reverb_control;
	//data->Resamplation  = ???;
}
...
static void savePreference(Pref_data* data)
{
    ...
    [dict setObject:[NSNumber numberWithInt:data->ModulationEnvelope] forKey: @"ModulationEnvelope"];
    [dict setObject:[NSNumber numberWithInt:data->TemperControl] forKey: @"TemperControl"];
    [dict setObject:[NSNumber numberWithInt:data->Reverb]        forKey: @"Reverb"];
	[dict setObject:[NSNumber numberWithInt:data->Resamplation]  forKey: @"Resamplation"];
...
void setPreference(Pref_data* data)
{
    ...
    opt_modulation_envelope = data->ModulationEnvelope;
    opt_temper_control = data->TemperControl;
    opt_reverb_control = data->Reverb;
    set_current_resampler(data->Resamplation);
...
void loadPreference()
{
    ...
    obj = [dict objectForKey:@"ModulationEnvelope"];
    if( obj ){  opt_modulation_envelope = [obj intValue]; }

    obj = [dict objectForKey:@"TemperControl"];
    if( obj ){  opt_temper_control = [obj intValue]; }
    
	obj = [dict objectForKey:@"Reverb"];
	if( obj ){ opt_reverb_control = [obj intValue]; }

	obj = [dict objectForKey:@"Resamplation"];
	if( obj ){
		pref_data.Resamplation = [obj intValue];
		set_current_resampler(pref_data.Resamplation);
	}
}
...
- (void)updateUI:(Pref_data*)data
{
    ...
    [i_ModulationEnvelope setState:data->ModulationEnvelope ];
    [i_TemperControl      setState:data->TemperControl ];
	[i_reverb             selectItemAtIndex:data->Reverb];
			[i_reverb synchronizeTitleAndSelectedItem];
	[i_resamplation       selectItemAtIndex:data->Resamplation];
			[i_resamplation synchronizeTitleAndSelectedItem];
}
...
- (void)getPreference_fromPanel:(Pref_data*)data
{
    ...
    data->ModulationEnvelope = [i_ModulationEnvelope state];
    data->TemperControl      = [i_TemperControl state];
	data->Reverb             = [i_reverb indexOfSelectedItem];
	data->Resamplation       = [i_resamplation indexOfSelectedItem];
}
